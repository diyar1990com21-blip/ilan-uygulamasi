...
# Kapsamlı ortak paket/versiyon havuzu; kullanıcı ayrıca özel paket girebilir.
COMMON_PACKAGES = ["Base", "Life", "Joy", "Touch", "Icon", "Zen", "Business", "Comfort", "Comfortline", "Highline", "Trendline", "Style", "Ambition", "Elegance", "Premium", "Luxury", "Sport", "M Sport", "R-Line", "GT Line", "Allure", "Active", "Allure Pack", "Titanium", "Vignale", "Platinum", "Dynamic", "Prestige", "Executive", "Ultimate", "Elite", "N Line", "Prime", "Dream", "Flame", "Passion", "Advance", "Impression", "Exclusive", "S", "SE", "SEL", "XLE", "L", "GLX", "Diğer / özel paket"]
YEARS = list(range(1980, 2027))
VEHICLE_DETAILS = {brand: {model: {"years": YEARS, "packages": COMMON_PACKAGES} for model in models} for brand, models in VEHICLES.items()}
...
def register(payload: RegisterPayload):
    ...
    if any(u["phone"] == phone for u in USERS):
        return JSONResponse({"ok": False, "message": "Bu telefon zaten kayıtlı."}, status_code=409)
    USERS.append({"id": str(uuid.uuid4()), "name": payload.name, "phone": phone, "role": payload.role, "verified": False, "otp": "1234", "password": payload.password})
    return {"ok": True, "message": "Doğrulama kodu gönderildi: 1234", "phone": phone}
...
@app.get("/api/config")
def config():
    return {"vehicles": VEHICLES, "vehicle_details": VEHICLE_DETAILS, "years": YEARS, "colors": COLORS, "categories": CATEGORIES}
...
function unlockApp(){sessionStorage.setItem('parca_auth','1');$('authGate').style.display='none';['create','pool','how'].forEach(id=>$(id).style.display='');toast('Kayıt tamamlandı. İlanlar ve mesajlaşma açıldı.')} function requireAuth(){if(sessionStorage.getItem('parca_auth')!=='1'){toast('Önce ücretsiz kayıt ve telefon doğrulaması yapın.');return false}return true} async function registerUser(){const name=prompt('Ad soyad / işletme adı:');if(!name)return;const role=(prompt('Rol: alici veya satici','alici')||'alici').toLowerCase().startsWith('sat')?'seller':'buyer';const phone=prompt('Cep telefonu:');if(!phone)return;const password=prompt('Şifre (en az 6 karakter):');if(!password)return;const r=await (await fetch('/api/auth/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,phone,role,password})})).json();toast(r.message);if(!r.ok)return;const code=prompt('SMS doğrulama kodu (demo: 1234):');if(!code)return;const v=await (await fetch('/api/auth/verify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({phone,code})})).json();toast(v.message);if(v.ok)unlockApp()} function scrollToId(id){$(id).scrollIntoView({behavior:'smooth'})} function toast(msg){const t=$('toast');t.textContent=msg;t.classList.remove('hidden');setTimeout(()=>t.classList.add('hidden'),3200)}
